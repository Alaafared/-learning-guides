...
function renderActiveFilters() {
  ...
  if (badge) {
    ...
    badge.style.display = chips.length ? 'inline-block' : 'none';
  }
    ...
    list.forEach(f => {
      ...
      row.innerHTML = `
        ...
        <div class="list-actions">
          <a href="${f.downloadLink}" target="_blank" rel="noopener" class="btn-download" style="padding:6px 12px;font-size:0.78rem;">
            <i class="fas fa-download"></i> تحميل
          </a>
        </div>
      `;
...
els.btnDownload.addEventListener('click', () => {
  ...
  if (isMobile && toDownload.length > 1) {
    // على الموبايل: فتح الملفات في تبويبات جديدة
    toast(`جاري فتح ${toDownload.length} ملف...`, 'info');
    toDownload.forEach((f, i) => {
      setTimeout(() => {
        window.open(f.downloadLink, '_blank');
      }, i * 800);
    });
    toast(`تم فتح ${toDownload.length} ملف في تبويبات جديدة`, 'success');
  } else {
    // سطح المكتب: تحميل عادي
    toDownload.forEach((f, i) => {
      setTimeout(() => {
    ...
    toast(`جاري تحميل ${toDownload.length} ملف...`, 'success');
  }
});
